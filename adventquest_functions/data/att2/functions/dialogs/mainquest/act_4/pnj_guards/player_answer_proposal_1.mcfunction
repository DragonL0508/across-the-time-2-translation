...
tellraw @a[scores={LANGUAGE=2}] {"selector":"@a[scores={NUMEROJOUEUR=1}]","extra":[{"text":" : "},{"text":"[You want to taste my blade? -->]","color":"gray","clickEvent":{"action":"run_command","value":"/execute if score kortaek_guards_PNJ DIALOG matches 1 run function att2:dialogs/mainquest/act_4/pnj_guards/answer_green_1"},"hoverEvent":{"action":"show_text","value":"Click here - answer 1"}}]}
