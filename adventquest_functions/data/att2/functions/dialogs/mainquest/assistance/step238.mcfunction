...
tellraw @a[scores={LANGUAGE=2}] {"selector":"@a[scores={NUMEROJOUEUR=1}]","color":"dark_red","extra":[{"text":" : "},{"text":"Security activated itself and another lock appeared in front of the main door. I have to go down and deactivate the security then look for another big key in order to get out of this building.","color":"aqua","bold":false}]}
