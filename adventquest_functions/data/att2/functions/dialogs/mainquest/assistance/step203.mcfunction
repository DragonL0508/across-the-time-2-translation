...
tellraw @a[scores={LANGUAGE=2}] {"selector":"@a[scores={NUMEROJOUEUR=1}]","color":"dark_red","extra":[{"text":" : "},{"text":"Skaolon considers that a Néleptron will not be enough to have an audience with the king. It begins to enrage me ... So be it! Let's go get another Neleptron, there should be another path leading there.","color":"aqua","bold":false}]}
