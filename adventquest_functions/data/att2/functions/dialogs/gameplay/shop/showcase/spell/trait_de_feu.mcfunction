...
tellraw @a[scores={LANGUAGE=2}] {"text":" ° ","bold":true,"color":"dark_red","extra":[{"text":"<Fire Line>","bold":false,"color":"red","hoverEvent":{"action":"show_text","value":"Fire the flames, incendiate those you are facing."},"extra":[{"text":" [350 Chronotons]","color":"yellow","clickEvent":{"action":"run_command","value":"/execute positioned as @e[tag=TRADER,distance=..7] at @s run function att2:gameplay/shop/purchase/spell/trait_de_feu"},"hoverEvent":{"action":"show_text","value":"Click here to buy!"}}]}]}
