...
tellraw @a[scores={LANGUAGE=2}] {"text":" ° ","bold":true,"color":"dark_red","extra":[{"text":"<Berserk>","bold":false,"color":"dark_green","hoverEvent":{"action":"show_text","value":"Draw from your vital resources to transfer them to your physical strength."},"extra":[{"text":" [350 Chronotons]","color":"yellow","clickEvent":{"action":"run_command","value":"/execute positioned as @e[tag=TRADER,distance=..7] at @s run function att2:gameplay/shop/purchase/spell/berserk"},"hoverEvent":{"action":"show_text","value":"Click here to buy!"}}]}]}
