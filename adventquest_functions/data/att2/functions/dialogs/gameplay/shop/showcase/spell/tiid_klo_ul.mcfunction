...
tellraw @a[scores={LANGUAGE=2}] {"text":" ° ","bold":true,"color":"dark_red","extra":[{"text":"<Tiid Klo Ul>","bold":false,"color":"red","hoverEvent":{"action":"show_text","value":"Create an aura around you for a while. Enemies trapped inside are slowed and weakened."},"extra":[{"text":" [400 Chronotons]","color":"yellow","clickEvent":{"action":"run_command","value":"/execute positioned as @e[tag=TRADER,distance=..7] at @s run function att2:gameplay/shop/purchase/spell/tiid_klo_ul"},"hoverEvent":{"action":"show_text","value":"Click here to buy!"}}]}]}
