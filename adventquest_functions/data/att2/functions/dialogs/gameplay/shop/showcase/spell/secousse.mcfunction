...
tellraw @a[scores={LANGUAGE=2}] {"text":" ° ","bold":true,"color":"dark_red","extra":[{"text":"<Earthquake>","bold":false,"color":"red","hoverEvent":{"action":"show_text","value":"Creates an earthquake wave spreading on the ground."},"extra":[{"text":" [1600 Chronotons]","color":"yellow","clickEvent":{"action":"run_command","value":"/execute positioned as @e[tag=TRADER,distance=..7] at @s run function att2:gameplay/shop/purchase/spell/secousse"},"hoverEvent":{"action":"show_text","value":"Click here to buy!"}}]}]}
