...
tellraw @a[scores={LANGUAGE=2}] {"text":" ° ","bold":true,"color":"dark_red","extra":[{"text":"<Swarm Arrow>","bold":false,"color":"red","hoverEvent":{"action":"show_text","value":"Invoke a swarm of fiery arrows above you, dealing heavy damage to all enemies within a certain radius."},"extra":[{"text":" [1000 Chronotons]","color":"yellow","clickEvent":{"action":"run_command","value":"/execute positioned as @e[tag=TRADER,distance=..7] at @s run function att2:gameplay/shop/purchase/spell/nuee_de_fleches"},"hoverEvent":{"action":"show_text","value":"Click here to buy!"}}]}]}
